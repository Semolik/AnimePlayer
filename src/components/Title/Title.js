    ...
    return ( <
        ...
        alt = { data.ru_title } > < /img> {
            ...
            data.genre &&
                ...
                div className = 'elements' > {
                    data.genre.map((element, key) => {
                        return <Link className = "genre"
                        to = { `/${service}/genre/${element[1]}` }
                        key = { key } > { element[0] } < /Link>
                    })
                } <
                /div> <
                /div>
            ...
            data.type && (data.type.length > 1 ?
                ...
                to = { `/${service}/genre/${data.type[1]}` } > < span > Тип < /span>{data.type[0]}</Link > : < div className = 'block' > < span > Тип < /span>{data.type[0]}</div >
            )
        } {
            data.year && (data.year.length > 1 ?
                <
                Link className = "block link"
                to = { `/${service}/genre/${data.year[1]}` } > < span > Год < /span>{data.year[0]}</Link > : < div className = 'block' > < span > Год < /span>{data.year[0]}</div >
            )
        } {
            data.director &&
                <
        ...
        /div> {
            data.series && data.series.data &&
                ...
                div className = 'flex w-100 margin-bottom' >
                <
                Plyr source = { data.series.data[0] }
            id = 'player'
            ref = {
            ...
            div className = 'series' > {
                ...
                div className = 'button-box-2' > {
                    data.series.data.map((element, key) => {
                            ...
                            // }
                            return ( < div className = { 'button' + (key === 0 ? ' active' : '') }
                                key = { key }
                                onClick = {
                                    (e) => {
